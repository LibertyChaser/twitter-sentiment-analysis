...
from mapping import EMOTICON_MAPPING, ABBREVIATION_MAPPING
...
def process_and_save_sentiment140():
    ...
    sentiment140_df = read_csv("training.1600000.processed.noemoticon.csv")
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    output_file_path = os.path.join(script_dir, '..', '..', 'data', 'processed', 'cleaned_training_sentiment140.csv')
...
def get_sentiment140():
    try:
        ...
        sentiment140_df = read_csv("cleaned_training_sentiment140.csv", folder='processed')
